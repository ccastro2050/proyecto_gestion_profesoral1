...
- Viene de un **registro** o se construye localmente. Este proyecto usa de
  ...
  `api_gestion/` (base: `dotnet/sdk:10.0`).
...
tiene 2: `./api_gestion`, `./front_blazor`.
...
WORKDIR /app
ENV DOTNET_USE_POLLING_FILE_WATCHER=1 ASPNETCORE_URLS=http://0.0.0.0:8074
EXPOSE 8074
CMD ["dotnet", "watch", "run", "--non-interactive", "--project", "ApiGestion.csproj"]
```
...
| `EXPOSE 8074` | **Documenta** en qué puerto escucha el programa | No abre nada: quien publica el puerto es el `ports:` del compose |
...
| **Bind mount** | Una carpeta de SU disco montada dentro del contenedor | `./api_gestion:/app` (el código entra al contenedor y `dotnet watch` lo vigila) · `./db:/scripts:ro` (los scripts del init, solo lectura) |
...
flowchart LR
    ...
    subgraph PC["Su PC — Docker Desktop (el 'centro de datos')"]
        subgraph RED["red interna del compose (LAN virtual, con DNS propio)"]
            FRONTBLAZOR["SERVIDOR WEB (front)<br/>contenedor front-blazor<br/>hostname: front-blazor · escucha en 8075"]
            APIGESTION["SERVIDOR DE APLICACIONES<br/>contenedor api-gestion<br/>hostname: api-gestion · escucha en 8074"]
    ...
    NAV -->|"localhost:8074"| APIGESTION
    NAV -->|"localhost:8075"| FRONTBLAZOR
    APIGESTION -->|"sqlserver:1433 (DNS de Docker)"| SQLSERVER
    FRONTBLAZOR -->|"http://api-gestion:8074<br/>al controlador, por el NOMBRE"| APIGESTION
    SQLSERVERINIT -->|"espera el healthcheck,<br/>siembra y termina"| SQLSERVER
    CLIENTE -.->|"opcional (diagnóstico):<br/>localhost:11472"| SQLSERVER
```
...
```yaml
  sqlserver:
    ...
    ports:
      - "11472:1433"                 # "puerto en su PC : puerto interno"
...
```yaml
  sqlserver-init:
    ...
    volumes:
      - ./db:/scripts:ro             # init.sh + gestion_local.sql, solo lectura
...
```yaml
  api-registros:
    build: ./api_gestion            # se construye con SU Dockerfile
    volumes:
      - ./api_gestion:/app          # guardar un .cs → dotnet watch recompila
...
1. **Dos redes de nombres**: hacia su PC, puertos publicados
   (`localhost:8074`, `localhost,11472`); entre contenedores, nombres de
...
docker network create proyecto_gestion_profesoral1_default

# 2. sqlserver
docker run -d --name gestion-sqlserver --network proyecto_gestion_profesoral1_default --restart unless-stopped `
  -e "ACCEPT_EULA=Y" `
  -e "MSSQL_SA_PASSWORD=Aplicacionweb123!" `
  -v mssqldata:/var/opt/mssql `
  -p 11472:1433 mcr.microsoft.com/mssql/server:2022-latest

# 3. ESPERAR a que responda de verdad… mirándolo a ojo

# 4. sqlserver-init
docker run -d --name gestion-sqlserver-init --network proyecto_gestion_profesoral1_default --restart no --entrypoint /bin/bash `
  -e "MSSQL_SA_PASSWORD=Aplicacionweb123!" `
  -v "${PWD}/db:/scripts:ro" mcr.microsoft.com/mssql/server:2022-latest /scripts/init.sh

# 5. Construir la imagen de api-gestion y encenderla
docker build -t api-gestion ./api_gestion
docker run -d --name gestion-api --network proyecto_gestion_profesoral1_default --restart unless-stopped `
  -e "ConnectionStrings__SqlServer=Server=sqlserver,1433;Database=gestion_local;User Id=sa;Password=Aplicacionweb123!;TrustServerCertificate=True;" `
  -v "${PWD}/api_gestion:/app" `
  -v /app/bin `
  -v /app/obj `
  -p 8074:8074 api-gestion

# 6. Construir la imagen de front-blazor y encenderla
docker build -t front-blazor ./front_blazor
docker run -d --name gestion-front --network proyecto_gestion_profesoral1_default --restart unless-stopped `
  -e "UrlApi=http://api-gestion:8074" `
  -v "${PWD}/front_blazor:/app" `
  -v /app/bin `
  -v /app/obj `
  -p 8075:8075 front-blazor
...
|---|---|---|
| **Su navegador** | `http://localhost:8074` | Está fuera de Docker. Usa el puerto **publicado** |
| **Otro contenedor** | `http://api-gestion:8074` | Están en la misma red: se llaman por el **nombre del servicio** |
...
> **En este proyecto los dos números son iguales** (`8074:8074`), y eso
...
Si alguien cambiara esa línea por `http://localhost:8074`, el front
...
```powershell
docker network inspect proyecto_gestion_profesoral1_default
```
...
`docker-compose.yml` y le pega `_default`. La carpeta `proyecto_gestion_profesoral1` produce
la red `proyecto_gestion_profesoral1_default`. Tres formas de confirmarlo:
...
docker inspect gestion-front --format '{{range $k,$v := .NetworkSettings.Networks}}{{$k}}{{end}}'
```
...
network proyecto_gestion_profesoral1_default has active endpoints (name:"gestion-front" …)
```
